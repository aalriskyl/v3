...
const axiosInstancePar = axios.create({
...
axiosInstancePar.interceptors.request.use((config) => {
...
axiosInstancePar.interceptors.response.use(
    (response) => response,
    (error) => {
        if (error.response) {
...
export default axiosInstancePar;
