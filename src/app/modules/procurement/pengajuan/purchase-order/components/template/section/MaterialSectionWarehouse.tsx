...
const MaterialTableSectionWarehouse: React.FC<{
    status: string;
    supplierId: string;
    materialRequestId: string;
    isQuotation: string;
}> = ({ status, supplierId, materialRequestId, isQuotation }) => {
    ...
    const columns = useMemo(() => materialColumnsWarehouse(status), [status]);
    ...
    return (
        <>
            <KTCardBody className="py-4">
                ...
                {/* Add Material Button */}
                {orderData?.status === "Draft" && orderData?.material_request_id == null && quotationData === null &&  (
...
export { MaterialTableSectionWarehouse };