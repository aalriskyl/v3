...
const FilterModal: FC<FilterModalProps> = ({ closeModal }) => {
    const { setSubmittedDate, setSubmittedDateTo, setApprovedDate, setApprovedDateTo, setStatus, submittedDate, submittedDateTo, approvedDate, approvedDateTo, dueDateFrom, dueDateTo, setDueDateFrom, setDueDateTo, requestedDate, requestedDateTo, setRequestedDate, setRequestedDateTo, status, statusPayment, setStatusPayment } = useFakturPembelian();
    ...
    const onSubmit: SubmitHandler<FormValues> = async (data) => {
        ...
        console.log(data);
        try {
            setDueDateFrom(formatDateForPayload(data.dueDateFrom));
            setDueDateTo(formatDateForPayload(data.dueDateTo));
            ...
            setStatusPayment(data.statusPayment);
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <DateRangeInputField
                            ...
                            endDate={control._formValues.dueDateTo} // Ambil nilai dueDateTo dari form
                            onChange={(start, end) => {
                                field.onChange(start); // Simpan nilai dueDateFrom
                                setValue("dueDateTo", end); // Simpan nilai dueDateTo menggunakan setValue
                            }}
                        />
                    )}
                />
