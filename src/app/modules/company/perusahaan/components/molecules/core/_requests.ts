...
import { Company, UsersQueryResponse } from "./_models";
...
const createUser = (user: Company ): Promise<Company  | undefined> => {
  return axios
    .put(USER_URL, user)
    .then((response: AxiosResponse<Response<Company >>) => response.data)
    .then((response: Response<Company >) => response.data);
...
const updateUser = (user: Company ): Promise<Company | undefined> => {
  return axios
