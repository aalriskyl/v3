...
const breadcrumbs: Array<PageLink> = [
  ...
  },
  {
    ...
    path: "/accounting/masterdata/payment-term",
    isSeparator: false,
    isActive: false,
  },
];
...
const DetailPage: FC = () => {
  ...
  return (
    <>
      ...
      <PageTitle breadcrumbs={breadcrumbs}>Detail Terms of Payment</PageTitle>
      {/* Display Brand Data */}
      <div className="container card p-5 font-secondar mb-8">
        <h4 className="mb-8">Terms of Payment</h4>
        <div className="row">
          {/* Kolom Kiri */}
...
const ListPaymentTerms = ({
  ...
  const paymentTermSchema = Yup.object().shape({
    name: Yup.string()
      .required("Nama Payment Terms wajib diisi")
      .max(255, "Nama Payment Terms maksimal 255 karakter")
      .matches(/^[A-Za-z0-9\s]+$/, "Nama Payment Terms tidak boleh mengandung karakter khusus"),
    invoice_portion: Yup.number()
      .required("Invoice Portion wajib diisi")
      .min(0, "Invoice Portion tidak boleh kurang dari 0")
      .max(100, "Invoice Portion tidak boleh lebih dari 100"),
    due_date_based_on: Yup.string().required("Due Date wajib diisi"),
    credit: Yup.number()
      .required("Credit wajib diisi")
      .min(0, "Credit tidak boleh kurang dari 0"),
  });
  const {
    control,
    handleSubmit,
  ...
  <input
    ...
    className={clsx("form-control py-4", {
      'is-invalid': errors.name, // Add 'is-invalid' class if there's an error
    })}
