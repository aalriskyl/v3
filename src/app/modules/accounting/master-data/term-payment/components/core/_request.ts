...
export const getTermsPayment = async (
  search: string = "",
  status?: string,
  page?: number,
  size?: number,
...
export const getService = async (
  ...
  try {
    const response = await axiosInstance.get(`/inventory/master-data/service?status=true`,
      {
        params: {
          page,
          size,
...
export const getAllServiceSupplierByServiceId = async (id: string) => {
  try {
    const response = await axiosInstance.get(
      `/inventory/master-data/service/service-supplier/service/${id}`
    );
    return response.data.data.service_suppliers;
  } catch (error) {
