...
const FilterModal: React.FC<FilterModalProps> = ({ closeModal }) => {
  const { dateFrom, dateTo, setDateFrom, setDateTo } = useJournal();
  ...
  const onSubmit: SubmitHandler<FormikValues> = async (data) => {
    ...
    try {
      ...
      setDateFrom(formatDateForPayload(data.dateFrom));
      setDateTo(formatDateForPayload(data.dateTo));
        ...
        <Controller
          ...
          render={({ field }) => (
            <DateRangeInputField
              ...
              endDate={control._formValues.dateTo} // Ambil nilai submittedTo dari form
              onChange={(start, end) => {
                field.onChange(start); // Simpan nilai submittedFrom
                setValue("dateTo", end); // Simpan nilai submittedTo menggunakan setValue
              }}
            />
          )}
        />
