...
const getUserById = (id: ID): Promise<Category | undefined> => {
  return axios
    .get(`${USER_URL}/${id}`)
...
const createUser = (user: Category): Promise<Category | undefined> => {
  return axios
    .put(USER_URL, user)
    .then((response: AxiosResponse<Response<Category>>) => response.data)
    .then((response: Response<Category>) => response.data);
...
const updateUser = (user: Category): Promise<Category | undefined> => {
  return axios
