// EntryStockContext.tsx (File terpisah)
import React, {
  createContext,
  useContext,
  useState,
...
interface EntryStockContextProps {
  data: any[];
  searchTerm: string;
  status: string;
...
const EntryStockContext = createContext<EntryStockContextProps | undefined>(
  undefined
);

export const EntryStockProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          setData(response.data.stock_entries || []);
...
export const useEntryStocks = () => {
  const context = useContext(EntryStockContext);
  if (!context) {
