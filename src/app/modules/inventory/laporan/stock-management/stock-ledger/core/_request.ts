...
//   },
// });
export type getStockLedgerType = {
  material_id: string;
  doc_type: string;
...
export const getAllStockLedgerByWarehouseId = async (
  warehouse_id: string,
  setTotalData?: React.Dispatch<React.SetStateAction<number>>,
  pageIndex?: number,
  pageSize?: number
): Promise<getStockLedgerType> => {
  try {
    const response = await axiosInstance.get(
      `/inventory/submission/stock-management/stock-history/stock-ledger?warehouse_id=${warehouse_id}&company_id=${localStorage.getItem(
        "company_id"
      )}`,
      {
        params: {
    ...
    console.log({
      getAllStockLedgerByWarehouseId: response.data.data.stock_histories,
    });
    if (setTotalData && pageSize) {
      setTotalData(response.data.data.total_page * pageSize);
    }
    return response.data.data.stock_histories;
  } catch (error) {
...
export type getAllWarehouseType = {
  id: string;
  name: string;
  address: string;
  company_id: string;
...
export const getAllWarehouse = async (): Promise<getAllWarehouseType> => {
  try {
    const response = await axiosInstance.get(
    ...
    console.log({ getAllWarehouse: response.data.data.warehouses }); // Log the response
